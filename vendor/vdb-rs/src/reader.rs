...
impl<R: Read + Seek> VdbReader<R> {
    ...
    fn read_tree_topology<ValueTy: Pod>(
        ...
        // PATCHED (see PATCHED.md): the background is a ValueTy, not 4 bytes.
        let mut _root_node_background_value = ValueTy::zeroed();
        reader.read_exact(bytes_of_mut(&mut _root_node_background_value))?;
        ...
        for _tile_idx in 0..number_of_tiles {
            ...
            // PATCHED (see PATCHED.md): a tile value is a ValueTy, not 4 bytes.
            let mut _value = ValueTy::zeroed();
            reader.read_exact(bytes_of_mut(&mut _value))?;
